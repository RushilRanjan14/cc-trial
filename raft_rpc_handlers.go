...
func (this *RaftNode) HandleRequestVote(args RequestVoteArgs, reply *RequestVoteReply) error {
	...
	if args.Term < this.currentTerm || (this.votedFor != -1 && this.votedFor != args.CandidateId) {
		reply.VoteGranted = false
	} else if this.lastLogTerm() > args.LastLogTerm || (this.lastLogTerm() == args.LastLogTerm && len(this.log)-1 > args.LastLogIndex) {
...
func (this *RaftNode) lastLogTerm() int {
	lastIndex := len(this.log) - 1
	if lastIndex < 0 {
		return -1
	}
	return this.log[lastIndex].Term
}
