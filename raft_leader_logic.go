			...
			if err := this.server.SendRPCCallTo(peerId, "RaftNode.AppendEntries", args, &reply); err == nil {
				...
				if this.state == "Leader" && termWhenHeartbeatSent == reply.Term {
					if reply.Success {
						...
						//-------------------------------------------------------------------------------------------/
						if reply.Success {
							// Update nextIndex and matchIndex for the received peer.
							this.matchIndex[peerId] = len(entries) + prevLogIndex
							this.nextIndex[peerId] = this.matchIndex[peerId] + 1
							// if this.log[this.matchIndex[peerId]].Term == this.currentTerm {
							// 	matchCount++
							// }
						}
