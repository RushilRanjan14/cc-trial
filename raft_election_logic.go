		...
		go func(peerId int) {
			...
			if err := this.server.SendRPCCallTo(peerId, "RaftNode.RequestVote", args, &reply); err == nil {
				...
				if reply.Term > this.currentTerm {
					// If the received term is greater than the node's current term,
					// update the node's term and become a follower.
					this.becomeFollower(reply.Term)
					return
				} else if reply.Term == this.currentTerm {
					// If the received term is equal to the node's current term,
					// check if the reply granted the vote.
					if reply.VoteGranted {
						votesReceived++
						if votesReceived > len(this.peersIds)/2 {
							// If the node has received a majority of votes, become a leader.
							this.becomeLeader()
							return
						}
					}
				} else {
					// If the received term is less than the node's current term,
					// ignore the reply.
					return
				}
				//-------------------------------------------------------------------------------------------/
...
func (this *RaftNode) becomeLeader() {
	this.write_log("became Leader with term=%d", this.currentTerm)
}
